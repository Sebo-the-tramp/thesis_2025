...
def main(
    ...
    gui_frames = server.gui.add_slider(
        ...
        max=len(images),
        step=1,
        initial_value=min(len(images), 800),
    )
    ...
    def visualize_frames() -> None:
        ...
        for img_id in tqdm(img_ids):
            ...
            frustum = server.scene.add_camera_frustum(
                f"/colmap/frames/frame_{img_id}/frustum",
        ...
        ##########################        
        pairs = []
        files = sorted(Path("./_assets/glb/").glob("*.glb"))
        every = 1
        i = 0
        for file in files:
            name = file.stem
            if "-" in name:
                if i % every == 0:
                    pairs.append(name.replace("_scene", "").split("-"))                    
            i+=1

        for pair in pairs:
            file_path = f"./_assets/glb/{pair[0]}-{pair[1]}.glb"
            print(file_path)
            ...
            # how can we dynamically choese the scale?
            # get points from the two cameras both in normal and denser point cloud
            cameraA_position_sparse = T_world_camera_metadata[pair[0]].translation()
            cameraB_position_sparse = T_world_camera_metadata[pair[1]].translation()
            cameraA_position_denser = np.array(scene.geometry["geometry_2"].vertices)[1]
            cameraB_position_denser = np.array(scene.geometry["geometry_4"].vertices)[1]
            distance_sparse = np.linalg.norm(cameraA_position_sparse - cameraB_position_sparse)
            distance_denser = np.linalg.norm(cameraA_position_denser - cameraB_position_denser)
            scale = distance_sparse / distance_denser

            new_dense_cloud_after = ((new_dense_cloud_after@ R.from_quat(T_world_camera_metadata[pair[0]].rotation().wxyz, scalar_first=True).as_matrix().T) * scale) + T_world_camera_metadata[pair[0]].translation()
